...
class AdminBloc extends ChangeNotifier {
  ...
  Future<List> getBannerList ()async{
    final DocumentReference ref = firestore.collection('banner').doc('banner');
    DocumentSnapshot snap = await ref.get();
    if(snap.exists == true){
      List bannerList = snap['banner'] ?? [];
      return bannerList;
    }
    else{
      await ref.set({
  ...
  Future addToBannerList (context, String timestamp) async {
    final DocumentReference ref = firestore.collection('banner').doc('banner');
    await getBannerList().then((bannerList)async{

      if (bannerList.contains(timestamp)) {
        openToast1(context, "This item is already available in the featured list");
      } else {

        bannerList.add(timestamp);
        await ref.update({'banner': FieldValue.arrayUnion(bannerList)});
        openToast1(context, 'Added Successfully');
      }
    });
  }

  Future removefromBannerList (context, String timestamp) async {
    final DocumentReference ref = firestore.collection('banner').doc('banner');
    await getBannerList().then((bannerList)async{

      if (bannerList.contains(timestamp)) {
        await ref.update({'banner' : FieldValue.arrayRemove([timestamp])});
      } else {
